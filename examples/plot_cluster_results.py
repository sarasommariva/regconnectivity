...
###############################################################################
# AUC values as function of the tested regularization parameter lam for the four connectivity metrics. In
# each panel, barplots and corresponding errorbars represent mean and standard deviation of the AUC values
# across the 108,000 simultations, while the x axis displays the value of the ratio between lam and the parameter
# lamX providing the best estimate of the neural activity. The red vertical line highlight when lam = lamX
...
###############################################################################
#  2D histogram showing the relationship between the optimal regularization parameters for different
# connectivity metrics. In each panel the x-axis shows the value of the optimal parameter for wPLI in
# logarithmic scale, while the y-axis refers to CPS, imCOH and ciPLV, respectively. Notice the different scale for
# the colorbar in each panel.
...
##################################################################################
# Impact of the measurement noise on the relationship between the optimal regularization
# parameters for different connectivity metrics. Each row refers to a different signal-to-noise ratio whose value
# is reported on top of the panels. Each column shows the 2D-histogram for a different pair of connectivity
# metrics, namely CPS vs wPLI (left column), imCOH vs wPLI (middle column), and ciPLV vs wPLI (right column)
...
for i_snr in range(N_snr):
    ...
    hist, yedges, xedges = np.histogram2d(np.log10(np.reshape(lambda_imcoh,(-1,))) ,np.log10(np.reshape(lambda_wpli,(-1,))), bins=[np.arange(-3,5.5,0.4),np.arange(-3,5.5,0.4)] )
    im1 = ax[i_snr,1].imshow(hist,extent=[xedges[0],xedges[-1],yedges[0],yedges[-1]], cmap=cmap, origin=origin)
    ...
    hist, yedges, xedges = np.histogram2d(np.log10(np.reshape(lambda_ciplv,(-1,))) ,np.log10(np.reshape(lambda_wpli,(-1,))), bins=[np.arange(-3,5.5,0.4),np.arange(-3,5.5,0.4)] )
    im1 = ax[i_snr,2].imshow(hist,extent=[xedges[0],xedges[-1],yedges[0],yedges[-1]], cmap=cmap, origin=origin)
...
for i_r in range(N_r):
    lambda_psds = tested_par[np.argmax(-opt_par['conn'][:,:,i_r,:,:,:,0,:],axis=-1)]*opt_par['tc'][:,:,i_r,:,:,:,0]
    lambda_imcoh = tested_par[np.argmax(-opt_par['conn'][:,:,i_r,:,:,:,1,:],axis=-1)]*opt_par['tc'][:,:,i_r,:,:,:,0]
    lambda_ciplv = tested_par[np.argmax(-opt_par['conn'][:,:,i_r,:,:,:,2,:],axis=-1)]*opt_par['tc'][:,:,i_r,:,:,:,0]
    
    lambda_wpli = tested_par[np.argmax(-opt_par['conn'][:,:,i_r,:,:,:,3,:],axis=-1)]*opt_par['tc'][:,:,i_r,:,:,:,0]
  
    hist, yedges, xedges = np.histogram2d(np.log10(np.reshape(lambda_psds,(-1,))) ,np.log10(np.reshape(lambda_wpli,(-1,))), bins=[np.arange(-3,5.5,0.4),np.arange(-3,5.5,0.4)] )
    im1 = ax[i_r,0].imshow(hist,extent=[xedges[0],xedges[-1],yedges[0],yedges[-1]], cmap=cmap, origin=origin)
    ...
    hist, yedges, xedges = np.histogram2d(np.log10(np.reshape(lambda_imcoh,(-1,))) ,np.log10(np.reshape(lambda_wpli,(-1,))), bins=[np.arange(-3,5.5,0.4),np.arange(-3,5.5,0.4)] )
    im1 = ax[i_r,1].imshow(hist,extent=[xedges[0],xedges[-1],yedges[0],yedges[-1]], cmap=cmap, origin=origin)
    ...
    hist, yedges, xedges = np.histogram2d(np.log10(np.reshape(lambda_ciplv,(-1,))) ,np.log10(np.reshape(lambda_wpli,(-1,))), bins=[np.arange(-3,5.5,0.4),np.arange(-3,5.5,0.4)] )
    im1 = ax[i_r,2].imshow(hist,extent=[xedges[0],xedges[-1],yedges[0],yedges[-1]], cmap=cmap, origin=origin)
...
for i_c in range(N_c):
    lambda_psds = tested_par[np.argmax(-opt_par['conn'][:,:,:,i_c,:,:,0,:],axis=-1)]*opt_par['tc'][:,:,:,i_c,:,:,0]
    lambda_imcoh = tested_par[np.argmax(-opt_par['conn'][:,:,:,i_c,:,:,1,:],axis=-1)]*opt_par['tc'][:,:,:,i_c,:,:,0]
    lambda_ciplv = tested_par[np.argmax(-opt_par['conn'][:,:,:,i_c,:,:,2,:],axis=-1)]*opt_par['tc'][:,:,:,i_c,:,:,0]
    
    lambda_wpli = tested_par[np.argmax(-opt_par['conn'][:,:,:,i_c,:,:,3,:],axis=-1)]*opt_par['tc'][:,:,:,i_c,:,:,0]
  
    hist, yedges, xedges = np.histogram2d(np.log10(np.reshape(lambda_psds,(-1,))) ,np.log10(np.reshape(lambda_wpli,(-1,))), bins=[np.arange(-3,5.5,0.4),np.arange(-3,5.5,0.4)] )
    im1 = ax[i_c,0].imshow(hist,extent=[xedges[0],xedges[-1],yedges[0],yedges[-1]], cmap=cmap, origin=origin)
    ...
    hist, yedges, xedges = np.histogram2d(np.log10(np.reshape(lambda_imcoh,(-1,))) ,np.log10(np.reshape(lambda_wpli,(-1,))), bins=[np.arange(-3,5.5,0.4),np.arange(-3,5.5,0.4)] )
    im1 = ax[i_c,1].imshow(hist,extent=[xedges[0],xedges[-1],yedges[0],yedges[-1]], cmap=cmap, origin=origin)
    ...
    hist, yedges, xedges = np.histogram2d(np.log10(np.reshape(lambda_ciplv,(-1,))) ,np.log10(np.reshape(lambda_wpli,(-1,))), bins=[np.arange(-3,5.5,0.4),np.arange(-3,5.5,0.4)] )
    im1 = ax[i_c,2].imshow(hist,extent=[xedges[0],xedges[-1],yedges[0],yedges[-1]], cmap=cmap, origin=origin)
...
for i_gamma in range(N_gamma):
    lambda_psds = tested_par[np.argmax(-opt_par['conn'][:,:,:,:,i_gamma,:,0,:],axis=-1)]*opt_par['tc'][:,:,:,:,i_gamma,:,0]
    lambda_imcoh = tested_par[np.argmax(-opt_par['conn'][:,:,:,:,i_gamma,:,1,:],axis=-1)]*opt_par['tc'][:,:,:,:,i_gamma,:,0]
    lambda_ciplv = tested_par[np.argmax(-opt_par['conn'][:,:,:,:,i_gamma,:,2,:],axis=-1)]*opt_par['tc'][:,:,:,:,i_gamma,:,0]
    
    lambda_wpli = tested_par[np.argmax(-opt_par['conn'][:,:,:,:,i_gamma,:,3,:],axis=-1)]*opt_par['tc'][:,:,:,:,i_gamma,:,0]
  
    hist, yedges, xedges = np.histogram2d(np.log10(np.reshape(lambda_psds,(-1,))) ,np.log10(np.reshape(lambda_wpli,(-1,))), bins=[np.arange(-3,5.5,0.4),np.arange(-3,5.5,0.4)] )
    im1 = ax[i_gamma,0].imshow(hist,extent=[xedges[0],xedges[-1],yedges[0],yedges[-1]], cmap=cmap, origin=origin)
    fig.colorbar(im1, ax=ax[i_gamma,0], location='right', shrink=shrink)
    ax[i_gamma,0].set_xlabel(r'log$_{10}(\lambda_\mathbf{wPLI})$')
    ax[i_gamma,0].set_title(str(np.round(bg_noise_levels[i_gamma],decimals=1)))
