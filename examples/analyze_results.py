...
ax[0, 0].set_title('S')
ax[0, 0].set_xlabel(r'$\lambda/\lambda_x$')
...
ax[0, 1].set_xlabel(r'$\lambda/\lambda_x$')
...
ax[1, 0].set_xlabel(r'$\lambda/\lambda_x$')
...
ax[1, 1].set_xlabel(r'$\lambda/\lambda_x$')
...
lambda_psds = tested_par[np.argmax(-opt_par['conn'][:, :, :, :, :, :, 0, :],
                                   axis=-1)]*opt_par['tc'][:, :, :, :, :, :, 0]
lambda_imcoh = tested_par[np.argmax(-opt_par['conn'][:, :, :, :, :, :, 1, :],
                                    axis=-1)]*opt_par['tc'][:, :, :, :, :, :,
                                                            0]
lambda_ciplv = tested_par[np.argmax(-opt_par['conn'][:, :, :, :, :, :, 2, :],
                                    axis=-1)]*opt_par['tc'][:, :, :, :, :, :,
                                                            0]
lambda_wpli = tested_par[np.argmax(-opt_par['conn'][:, :, :, :, :, :, 3, :],
                                   axis=-1)]*opt_par['tc'][:, :, :, :, :, :, 0]
...
origin = 'lower'
cmap = 'jet'
hist, yedges, xedges = np.histogram2d(np.log10(np.reshape(lambda_psds, (-1,))),
                                      np.log10(np.reshape(lambda_wpli, (-1,))),
                                      bins=[np.arange(-3, 5.5, 0.4),
                                      np.arange(-3, 5.5, 0.4)])
im1 = ax[0].imshow(hist, extent=[xedges[0], xedges[-1],
                   yedges[0], yedges[-1]], cmap=cmap, origin=origin)
...
hist, yedges, xedges = np.histogram2d(np.log10(np.reshape(lambda_imcoh,
                                               (-1,))),
                                      np.log10(np.reshape(lambda_wpli, (-1,))),
                                      bins=[np.arange(-3, 5.5, 0.4),
                                      np.arange(-3, 5.5, 0.4)])
im2 = ax[1].imshow(hist, extent=[xedges[0], xedges[-1], yedges[0], yedges[-1]],
...
hist, yedges, xedges = np.histogram2d(np.log10(np.reshape(lambda_ciplv,
                                               (-1,))),
                                      np.log10(np.reshape(lambda_wpli, (-1,))),
                                      bins=[np.arange(-3, 5.5, 0.4),
                                      np.arange(-3, 5.5, 0.4)])
im3 = ax[2].imshow(hist, cmap=cmap, extent=[xedges[0],
                   xedges[-1], yedges[0], yedges[-1]], origin=origin)
...
for i_snr in range(N_snr):
    lambda_psds = tested_par[np.argmax(-opt_par['conn'][:, :, :, :, :, i_snr,
                                                        0, :], axis=-1)] *\
                  opt_par['tc'][:, :, :, :, :, i_snr, 0]
    lambda_imcoh = tested_par[np.argmax(-opt_par['conn'][:, :, :, :, :, i_snr,
                                                         1, :], axis=-1)] *\
                   opt_par['tc'][:, :, :, :, :, i_snr, 0]
    lambda_ciplv = tested_par[np.argmax(-opt_par['conn'][:, :, :, :, :, i_snr,
                                                         2, :], axis=-1)] *\
                   opt_par['tc'][:, :, :, :, :, i_snr, 0]
    lambda_wpli = tested_par[np.argmax(-opt_par['conn'][:, :, :, :, :, i_snr,
                                                        3, :], axis=-1)] *\
                  opt_par['tc'][:, :, :, :, :, i_snr, 0]

    hist, yedges, xedges = np.histogram2d(np.log10(np.reshape(lambda_psds,
                                                   (-1,))),
                                          np.log10(np.reshape(lambda_wpli,
                                                   (-1,))),
                                          bins=[np.arange(-3, 5.5, 0.4),
                                          np.arange(-3, 5.5, 0.4)])
    im1 = ax[i_snr, 0].imshow(hist, extent=[xedges[0], xedges[-1], yedges[0],
                              yedges[-1]], cmap=cmap, origin=origin)
    ...
    hist, yedges, xedges = np.histogram2d(np.log10(np.reshape(lambda_imcoh,
                                          ...
                                          bins=[np.arange(-3, 5.5, 0.4),
                                          np.arange(-3, 5.5, 0.4)])
    im2 = ax[i_snr, 1].imshow(hist, extent=[xedges[0], xedges[-1], yedges[0],
                              yedges[-1]], cmap=cmap, origin=origin)
    ...
    hist, yedges, xedges = np.histogram2d(np.log10(np.reshape(lambda_ciplv,
                                          ...
                                          bins=[np.arange(-3, 5.5, 0.4),
                                          np.arange(-3, 5.5, 0.4)])
    im3 = ax[i_snr, 2].imshow(hist, cmap=cmap, extent=[xedges[0], xedges[-1],
                              yedges[0], yedges[-1]], origin=origin)
    ...
    ax[i_snr, 1].set_title(
        r'SNR='+str(np.round(SNR_val[i_snr], decimals=1))+'\n')
fig.show()
