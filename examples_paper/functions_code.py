...
# select N_pos pairs of active sources, given the leadfield (G) and the source locations (src),
# so that the distance between the two sources is grater than 3 cm (0.03 m) and the intensity at sensor level is similar 

def select_sources(G,src,N_loc):
    """
    Select pairs of sources in the source space
...
def gen_ar_model(N_act, P, Sigma):
    ...
    index = np.where(I==1)
    vect=(np.concatenate((vect[0],index[0])),np.concatenate((vect[1],index[1])))
...
# %%
"""not used anymore"""
def get_sigma(sig,N_tot, A):
    val = 0
    for i in range(N_tot):
        val = val + np.exp(-0.5*i**2/sig**2)
    return val-A

# given the seed time course generate gaussian patch with specific intra coherence
def gen_coherent_patches(seed_tc, p1_locs, p2_locs, c,i_c, nperseg, nfft, fs, fmin, fmax):
    
    """
    Generate time courses to be assigned to the sources within the patches. The patches are Gaussian and have specific intracoherence level
    ...
    std_dev1 = (len(p1_locs)-1)/(np.sqrt(-2*np.log(0.4))) # standard deviation patch 1
    w1 = signal.gaussian(len(p1_locs)*2-1,std_dev1) # weights patch 1
    w1 = w1[(len(p1_locs)-1):]
    std_dev2 = (len(p2_locs)-1)/(np.sqrt(-2*np.log(0.4))) # standard deviation patch 2
    ...
    if c==1:
        ...
        if c == 0.5: rate1=100; rate2=100
        elif c== 0.2: rate1=500; rate2=300
        else: print('wrong coherence level, accepted values are: 1, 0.5, 0.2')
        
        # grow patch 1
        for i_source in range(1,len(p1_locs)):
            mean_coh = np.array([0,1])
            while (np.min(mean_coh)<c-0.2 or np.max(mean_coh)>c+0.2):
                new_tc = fft(seed_tc[0,:].reshape((1,-1))) + np.random.randn(1,T)*(np.random.randn(1)* rate1+rate2)
                new_tc = np.real(ifft(new_tc))*w1[i_source]*(np.linalg.norm(seed_tc[0,:])/np.linalg.norm(np.real(ifft(new_tc))))
                
                f, conn = signal.coherence(p1_tcs, new_tc, fs=fs, window='hann', nperseg=nperseg,
                                           noverlap=nperseg//2, nfft=nperseg, detrend='constant', axis=-1)
        ...
        for i_source in range(1,len(p2_locs)):
            mean_coh = np.array([0,1])
            while (np.min(mean_coh)<c-0.2 or np.max(mean_coh)>c+0.2):
                new_tc = fft(seed_tc[1,:].reshape((1,-1))) + np.random.randn(1,T)*(np.random.randn(1)* rate1+rate2)
                new_tc = np.real(ifft(new_tc))*w2[i_source]*(np.linalg.norm(seed_tc[1,:])/np.linalg.norm(np.real(ifft(new_tc))))
                
                f, conn = signal.coherence(p2_tcs, new_tc, fs=fs, window='hann', nperseg=nperseg,
                                           noverlap=nperseg//2, nfft=nperseg, detrend='constant', axis=-1)
...
def err_X_rel(Lambda, X, Y, G, GGt):
    """
    Error in estimating X using the regularization parameter Lambda
              _                       2
              \   ||X_Lambda(t)-X(t)||  
              /_t
     err_X = ______________________________
             _                   2        2
             \   (||X_Lambda(t)||+||X(t)|| )
             /_t
    ...
    if Lambda<0:
        ...
        I = np.eye(G.shape[0])
        W_tik = G.T.dot(np.linalg.inv(GGt+Lambda*I))
        value = np.linalg.norm(W_tik.dot(Y)-X, ord='fro')**2/(np.linalg.norm(W_tik.dot(Y), ord='fro')**2+np.linalg.norm(X, ord='fro')**2)
    return value
...
"""not used anymore"""
def err_S(Lambda, s, u_SY, v_SX, NN):
    if Lambda<0:
        value = np.inf
    else:
        MM = u_SY.shape[0]
        ...
        for ii in range(MM):
            for jj in range(MM):
                Phi[ii,jj] = (s[ii]**2/(Lambda + s[ii]**2))*(s[jj]**2/(Lambda + s[jj]**2))
        value = 0
        for k in range(nfft):
            aux_u = u_SY[:,:,k]
            aux_v = v_SX[:,:,k]
            value = value + np.linalg.norm(Phi*aux_u-aux_v,ord='fro')**2
        value = value/(NN**2*nfft)
    return value
...
def auc(Lambda, method, G, GGt, Y, p1_sources, p2_sources, fmin, fmax, PN_matrix,fs,nperseg):
    """
    ...
    Parameters:
        ...
        method: string | list/tuple of strings
            connectivity metrics for which compute the AUC. Valid methods are 'cpsd',
        ...
        PN_matrix: array, shape (N_p1, N_dense-N_p1)
            matrix indicating which sources are connected (i.e. sources within patch 1 with sources within patch 2)
        fs: float
            sampling frequency
        nperseg: int
            lenght of the epochs for the computation of the coherence
    ...
    Returns:
        ...
        TPF_all: array, shape (number of connectivity metrics, 20)
            True positive fraction for each connectivity metric and for each threshold level
        FPF_all: array, shape (number of connectivity metrics, 20)
            False positive fraction for each connectivity metric and for each threshold level
    """
    if Lambda < 0:
        value = 0
    else:
        if type(method) is str: method = [method]
        ...
        if 'cpsd' in method:
            ...
            for k in range(len(p1_sources)):
                f, Connlam_row = signal.csd(X_lam[p1_sources[k],:], X_lam, fs=fs, window='hann',
            ...
            Conn_lam[method.index('cpsd')] = np.delete(conn_cpsd,p1_sources,axis=-1)
                
        else: method_conn = method
        ...
        if len(method_conn) > 0:
            ...
            for i_epo in range(nepo):
                X_lam_re[i_epo,:,:] = X_lam[:,(nperseg-noverlap)*i_epo:(nperseg-noverlap)*i_epo+nperseg]
            
           
            indices = mne.connectivity.seed_target_indices(p1_sources,np.arange(N))
            (Conn, f, time,n_epochs, n_taper) = mne.connectivity.spectral_connectivity(X_lam_re, method=method_conn,mode='fourier',
                                                                           indices=indices,
                                                                           sfreq=fs,fmin=fmin, fmax=fmax, faverage=False, verbose=False)
            for i_conn in range(len(method_conn)):
                Conn_lam[method.index(method_conn[i_conn])] = np.reshape(abs(Conn[i_conn]).mean(axis=-1).copy(), (len(p1_sources),N))
                Conn_lam[method.index(method_conn[i_conn])] = np.delete(Conn_lam[method.index(method_conn[i_conn])],p1_sources,axis=-1)
        ...
        for i_conn in range(len(method)):
            ...
            for i_alp in range(len(Alpha)):
                aux = np.where(abs(Conn_lam[i_conn])>=Alpha[i_alp]*np.max(abs(Conn_lam[i_conn])) , abs(Conn_lam[i_conn]), 0)
                PN_evaluate = np.where(aux==0 , aux, 1)
                
                TP = np.count_nonzero(PN_matrix == np.where(PN_evaluate==1, PN_evaluate , -1))
                FP = np.count_nonzero(PN_evaluate)-TP
                #print(TP,FP)
                FPF[i_alp] = FP/(PN_matrix.shape[0]*PN_matrix.shape[1]-np.count_nonzero(PN_matrix))
