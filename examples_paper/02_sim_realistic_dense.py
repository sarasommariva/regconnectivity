...
fwd = convert_forward_solution(fwd, surf_ori=True,force_fixed=True, use_cps=True, verbose=False)
...
fmin = features['fmin']
fmax = features['fmax']
patch_radii = np.sqrt((np.array([2,4,8])*10**(-4))/math.pi) # radius of the patch (maximum distance from the seed in meters)
...
parameters = {'tc':np.zeros((len(patch_radii),len(coh_levels),len(bg_noise_levels),N_snr, 4)), 
              'tc_AUC': np.zeros((len(patch_radii),len(coh_levels),len(bg_noise_levels),N_snr, 2, len(lambdas))),
              'conn': np.zeros((len(patch_radii),len(coh_levels),len(bg_noise_levels),N_snr, 4, len(lambdas))),
              'TPF_conn': np.zeros((len(patch_radii),len(coh_levels),len(bg_noise_levels),N_snr, len(lambdas),4,20)),
              'FPF_conn': np.zeros((len(patch_radii),len(coh_levels),len(bg_noise_levels),N_snr, len(lambdas),4,20)),
              'sigma_noise':np.zeros((len(patch_radii),len(coh_levels),len(bg_noise_levels),N_snr))}

# initialize matrix to store spectral complecxity levels
spectal_complexity_Y = np.zeros((len(patch_radii),len(coh_levels),len(bg_noise_levels),N_snr))
...
for r in patch_radii:
    ...
    for c in coh_levels: 
        ...
        sys.stdout.flush()
        tic  = time.time()
        p1_tcs, p2_tcs = myf.gen_coherent_patches(seed_tc, p1_locs, p2_locs, c,i_c, nperseg, nfft, fs, fmin, fmax)
        toc = time.time()
        ...
        for Gamma in bg_noise_levels:
            ...
            bg_tcs = bg_tcs_general*np.sqrt((patches_norm/bg_norm_general)*Gamma)
            ...
            for Alpha in SNR_val:
                ...
                Sigma = np.sqrt(np.linalg.norm(G.dot(X), ord='fro')**2/(10**(Alpha/10)*np.linalg.norm(N_tilde, ord='fro')**2))
                N = Sigma*N_tilde
                ...
                spectal_complexity_Y[i_r,i_c,i_gamma,i_snr] = myf.spectral_complexity(Y, fs, nperseg, fmin, fmax)
                
                # define the matrix of positives and negatives (positives=1, negtives=0) for neural activity
                ...
                X_filt = signal.filtfilt(b, a, X, axis=- 1, padtype='odd', padlen=None, method='pad', irlen=None)
                Y_filt = signal.filtfilt(b, a, Y, axis=- 1, padtype='odd', padlen=None, method='pad', irlen=None)
                ...
                opt_set = optimize.minimize(myf.err_X,input_lamX, args=(X_filt,Y_filt,G,GGt),method='Nelder-Mead')
                lamX_alpha = opt_set['x'][0].copy()
                
                # lambda X rel
                #opt_set = optimize.minimize(myf.err_X_rel,input_lamX, args=(X,Y,G,GGt),method='Nelder-Mead')
                #lamX_rel = opt_set['x'][0].copy()
                
                # lambda X rel alpha range
                #opt_set = optimize.minimize(myf.err_X_rel,input_lamX, args=(X_filt,Y_filt,G,GGt),method='Nelder-Mead')
                #lamX_rel_alpha = opt_set['x'][0].copy()
                toc = time.time()
                ...
                tic = time.time()
                sys.stdout.flush()
                #aucs_tc = np.zeros((len(lambdas)))
                #aucs_tc_filt = np.zeros((len(lambdas)))
                TPF_conn = np.zeros((len(lambdas),4,20)) # lambdas*conn_meths*thresholds
                FPF_conn = np.zeros((len(lambdas),4,20)) # lambdas*conn_meths*thresholds
                AUC_conn = np.zeros((len(lambdas),4))
                for i_lam in range(len(lambdas)):
                    #aucs_tc[i_lam] = myf.auc_X(lambdas[i_lam]*lamX, G,GGt, Y, p1_locs, p2_locs, PN_matrix_tc)
                    #aucs_tc_filt[i_lam] = myf.auc_X(lambdas[i_lam]*lamX, G,GGt, Y_filt, p1_locs, p2_locs, PN_matrix_tc)
                    AUC_conn[i_lam,:], TPF_conn[i_lam,:,:], FPF_conn[i_lam,:,:], = myf.auc(lambdas[i_lam]*lamX, ['cpsd','imcoh','ciplv','wpli'], G, GGt,Y, p1_locs, p2_locs, fmin, fmax, PN_matrix_conn,fs,nperseg)
                    
                parameters['tc'][i_r,i_c,i_gamma,i_snr, 0] = lamX.copy()
                parameters['tc'][i_r,i_c,i_gamma,i_snr, 1] = lamX_alpha.copy()
                #parameters['tc'][i_r,i_c,i_gamma,i_snr, 2] = lamX_rel.copy()
                #parameters['tc'][i_r,i_c,i_gamma,i_snr, 3] = lamX_rel_alpha.copy()
                #parameters['tc_AUC'][i_r,i_c,i_gamma,i_snr,0,:] = aucs_tc.copy()
                #parameters['tc_AUC'][i_r,i_c,i_gamma,i_snr,1,:] = aucs_tc_filt.copy()
                parameters['conn'][i_r,i_c,i_gamma,i_snr, 0,:] = AUC_conn[:,0].copy()
                parameters['conn'][i_r,i_c,i_gamma,i_snr, 1,:] = AUC_conn[:,1].copy()
                parameters['conn'][i_r,i_c,i_gamma,i_snr, 2,:] = AUC_conn[:,2].copy()
                parameters['conn'][i_r,i_c,i_gamma,i_snr, 3,:] = AUC_conn[:,3].copy()
                parameters['TPF_conn'][i_r,i_c,i_gamma,i_snr, :,:,:] = TPF_conn.copy()                
                parameters['FPF_conn'][i_r,i_c,i_gamma,i_snr, :,:,:] = FPF_conn.copy()
                
                toc = time.time()
                print('time for computing optimal parameters for connectivity:'+str(toc-tic))
...
np.save('./run_data/'+str(job_run+1)+'/opt_parameters_loc'+str(i_loc)+'_mod'+str(i_mod)+'_i_run'+str(job_run+1)+'.npy', parameters)
np.save('./run_data/'+str(job_run+1)+'/spectal_complexity_Y_loc'+str(i_loc)+'_mod'+str(i_mod)+'_i_run'+str(job_run+1)+'.npy', spectal_complexity_Y)
if (i_loc == 0 & i_mod == 0): 
